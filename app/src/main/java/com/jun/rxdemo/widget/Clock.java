...
public class Clock extends View {
    ...
    private TimerTask task = new TimerTask() {
        ...
        public void run() {
            ...
            //具体计算
            mS = mS + 6;
            mM = mM + 0.1f;//每一秒，分钟走0.1f度
            mH = mH + 1.0f/120;//每一秒， 时针走120分之1度
            //子线程用postInvalidate
            postInvalidate();
        }
    };
    ...
    public Clock(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context,attrs);//初始化自定义参数
        initPaint();//初始化画笔  时钟画笔  指针画笔  数字画笔
        ...
        LogUtil.e("--hours:"+hours+"----minutes:"+minutes+"---seconds:"+seconds);
        setTime(hours,minutes,seconds);//计算时间  时 分  秒的刻度
        //开启定时
        start();
    }
    ...
    private int getMeasureSize(boolean isWidth, int measureSpec) {
        ...
        switch (specMode) {
            ...
            case MeasureSpec.AT_MOST://WrapContent
                ...
                break;
            case MeasureSpec.EXACTLY://MatchParent
                result = specSize;
                break;
        }
        return result;
